...
class WidgetFilm:
    ...
    def __event_open_add_form(self) -> None:
        ...
        #
        self.__form.pbOk.clicked.connect(self.__event_ok)
        self.__form.pbCancel.clicked.connect(self.__event_close)
    ...
    def __event_open_edit_form(self, row: int, column: int) -> None:
        ...
        self.row = row
        data = self.get_select_data(row, 0)
        print(data)
        self.__form.addWidgets(data)
        # #
        # notification = UiNotification()
        # notification.add("test", "test")
        # События нажатия на кнопки
        # self.__form.pbOk.clicked.connect()
        #
        self.__form.pbOk.clicked.connect(self.__event_ok)
        self.__form.pbCancel.clicked.connect(self.__event_close)

    def __event_ok(self) -> None:
        print("ok! save")
        # length = len(self.data)
        # self.data = (self.data[length-1], *self.data[:length-1])
        # data = self.get_select_data(self.row, 0)
        # print(data)
        # updater(data, self.__db)

    def __event_close(self) -> None:
        print("close")
        self.__form.close()
    ...
    def get_select_data(self, row: int, count: int) -> tuple:
